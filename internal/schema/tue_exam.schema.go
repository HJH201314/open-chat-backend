...
type Exam struct {
	...
	ExamProblems []ExamProblem `gorm:"foreignKey:ExamID;references:ID" json:"problems"` // 考试包含的大题
...
type ExamProblem struct {
	...
	Score     uint64  `gorm:"not null" json:"score"`                            // 题目分值（1表示0.01分）
	SortOrder int     `gorm:"not null" json:"sort_order"`                       // 题目排序
}
